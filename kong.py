...
def main():
    ...
    parentp.add_argument("--verbose", "-v", action="count")

    peekp = subparsers.add_parser("peek", parents=[parentp])
    peekp.set_defaults(func=peek)
    peekp.add_argument("stream", choices=["out", "err"])
    peekp.add_argument("jobid", type=int)
    peekp.add_argument("subjobid", type=int)

    submitp = subparsers.add_parser("submit", parents=[parentp])
    submitp.set_defaults(func=cli_submit)
    submitp.add_argument("lists", nargs="+")
    submitp.add_argument("--name")
    submitp.add_argument("--dry-run", action="store_true")

    # treep = subparsers.add_parser("tree", parents=[parentp])
    # treep.set_defaults(func=tree)

    lsp = subparsers.add_parser("ls", parents=[parentp])
    lsp.set_defaults(func=ls)
    lsp.add_argument("dir", nargs="?", default=os.getcwd())
    lsp.add_argument("--all", "-a", action="store_true")
    lsp.add_argument("--force", "-f", action="store_true")

    rmp = subparsers.add_parser("rm", parents=[parentp])
    rmp.set_defaults(func=rm)
    rmp.add_argument("tgt", nargs="+")

    killp = subparsers.add_parser("kill", parents=[parentp])
    killp.set_defaults(func=kill)
    killp.add_argument("tgt", nargs="+")

    recoverp = subparsers.add_parser("recover", parents=[parentp])
    recoverp.set_defaults(func=recover)
    recoverp.add_argument("job", type=int)

    resubmitp = subparsers.add_parser("resubmit", parents=[parentp])
    resubmitp.set_defaults(func=resubmit)
    resubmitp.add_argument("tgt", nargs="+")
    resubmitp.add_argument("--status", "-s", choices=["done", "pend", "exit", "run"], default="exit")
    resubmitp.add_argument("--force", "-f", action="store_true")

    viewp = subparsers.add_parser("view", parents=[parentp])
    viewp.set_defaults(func=view)
    viewp.add_argument("tgt", nargs="+")
    viewp.add_argument("--force", "-f", action="store_true")
    viewp.add_argument("--status", "-s", choices=["done", "pend", "exit", "run"])
...
def tree(args, config):
    kongdir, regdir, outdir = get_directories(config)

    os.system("tree {}".format(regdir))
