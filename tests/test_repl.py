...
def test_wait(repl, state, monkeypatch):
    ...
    repl.onecmd("wait * --no-notify --recursive --poll-interval 1h")
    wait.assert_called_once_with(
        ("*",),
        notify=False,
        recursive=True,
    ...
    repl.onecmd("wait * --no-notify --recursive")
    wait.assert_called_once_with(
        ("*",), notify=False, recursive=True, poll_interval=None, update_interval=None
    )
    wait.reset_mock()
...
def test_tail(state, repl, capsys, monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        res = iter([False, False, True])

        def exists(f):
            if f == job.data["stdout"]:
                return next(res)

        m.setattr("os.path.exists", Mock(side_effect=exists))
        spinner = MagicMock()
